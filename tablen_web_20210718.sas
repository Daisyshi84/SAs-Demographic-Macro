...
%macro tablen(
    ...
    %if %sysevalf(%superq(rowby)^=,boolean) %then %do;
        ...
        proc sql noprint;
            ...
            %if %sysevalf(%superq(rowbyorder)^=,boolean) %then %do;
                update _rowbyvar
                    set _rowbylvl_=case(prxchange('s/\s/ /', -1,_rowby_))
    ...
    %if %sysevalf(%superq(colby)^=,boolean) %then %do;
        ...
        proc sql noprint;
            ...
            %if %sysevalf(%superq(colbyorder)^=,boolean) %then %do;
                update _colbyvar
                    set _colbylvl_=case(prxchange('s/\s/ /', -1,_colby_))
    ...
    %if %sysevalf(%superq(by)^=,boolean) %then %do;
        ...
        proc sql noprint;
            ...
            %if %sysevalf(%superq(byorder)^=,boolean) %then %do;
                update _byvar
                    set _bylvl_=case(prxchange('s/\s/ /', -1,_by_))
            ...
            proc sql noprint;
                update _combined
                    set 
                        %do i = 1 %to &_ndisvars_;
                            %if &i > 1 %then %do; , %end;  
                            _dis_&i._lvl=case(prxchange('s/\s/ /', -1, _dis_&i))
                    ...
                    %if %sysevalf(%scan(&type,&i,%str( ))^=2,boolean) %then %do;
                        do i = 1 to dim(_ndisplay_&i._);         
                            ...
                            if ^(&nby=1 and ((vartype=4 and _ndisplay_&i._(i) in('Hazard Ratio (95% CI)' 'Wald P-value')) or
                               ...
                               and ^(vartype in(1 4) and dim(_ndisplay_&i._)=1) then output _tester_;
                        end;
    ...
    %if %sysfunc(countw(%scan(%superq(contdisplay),&i,|,m),%str( )))=1 %then %do;
        %local contfactor&i;        
          ...
          %if &_nnumvars_>0 %then %do;
            ...
            from (select *,
                ...
                %do i = 1 %to &_ndisvars_;
                    when vartypenum=&i and varlevel=.l then 
                           %if %sysfunc(countw(%scan(%superq(dis_order),&i,|,m),%str( )))=1 and %sysfunc(find(%scan(%superq(dis_order),&i,|,m),freq,i))=0 %then %do; 1 %end;
                           %else %do; varlevel %end;
                    when vartypenum=&i then 
                           %if %sysfunc(countw(%scan(%superq(dis_order),&i,|,m),%str( )))=1 and %sysfunc(find(%scan(%superq(dis_order),&i,|,m),freq,i))=0 %then %do; .e %end;
                           %else %do; varlevel %end;
           ...
           %if &_nsurvvars_>0 %then %do;
            ...
            from (select *,
                ...
                %do i = 1 %to &_nsurvvars_;
                    when vartypenum=&i then 
                           %if %sysfunc(countw(%scan(%superq(survdisplay),&i,|,m),%str( )))=1 %then %do; "&&survfactor&i" %end;
                           %else %do; factor %end;
