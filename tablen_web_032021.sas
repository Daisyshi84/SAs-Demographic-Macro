...
%macro tablen(
    ...
    %if %sysevalf(%superq(rowby)^=,boolean) %then %do;
        ...
        proc sql noprint;
            ...
            %if %sysevalf(%superq(rowbyorder)^=,boolean) %then %do;
                update _rowbyvar
                    set _rowbylvl_=case(tranwrd(_rowby_,'09'x,' '))
    ...
    %if %sysevalf(%superq(colby)^=,boolean) %then %do;
        ...
        proc sql noprint;
            ...
            %if %sysevalf(%superq(colbyorder)^=,boolean) %then %do;
                update _colbyvar
                    set _colbylvl_=case(tranwrd(_colby_,'09'x,' '))
    ...
    %if %sysevalf(%superq(by)^=,boolean) %then %do;
        ...
        proc sql noprint;
            ...
            %if %sysevalf(%superq(byorder)^=,boolean) %then %do;
                update _byvar
                    set _bylvl_=case(tranwrd(_by_,'09'x,' '))
            ...
            proc sql noprint;
                update _combined
                    set 
                        %do i = 1 %to &_ndisvars_;
                            %if &i > 1 %then %do; , %end;  
                            _dis_&i._lvl=case(tranwrd(_dis_&i,'09'x,' '))
    ...
    %if &_ndisvars_>0 %then %do;
        ...
        proc freq data=_discrete noprint;
            ...
            table _dislvl_*_bylvl_ / outpct sparse out=_dis_stat;
            weight _freq_ / zeros;
        run;
        %if &nby>1 %then %do;
